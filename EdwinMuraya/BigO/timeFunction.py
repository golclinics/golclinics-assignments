...
def is_palindrome_rev2(s):
    reversed_seq = reversed(
        s
    )  # creates a sequence with the characters in the string in reverse
    reversed_s = "".join(
        reversed_seq
    )  # transformed the reversed sequence into a string
    return s == reversed_s
...
s = (
    "m" * 500 + "u" + "m" * 500
)  # this creates a string with 1001 characters, 500 'm' followed by a 'u' followed by 500 'm'.
